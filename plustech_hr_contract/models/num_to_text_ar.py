...
def amount_to_text_arabic(number, currency):
    ...
    if real_number < 0 :
        ...
        if int(list[1]) > 0:
            return u'فقط '+ ' ' + u'سالب' + ' ' + start_word + ' ' + _get_currency_name_by_code(currency)[0] + ' ' + u'و' + ' ' + end_word + " " + cents_name
        else:
            return u'فقط '+ ' ' + u'سالب' + ' ' +  start_word + ' ' + _get_currency_name_by_code(currency)[0]
    else :
        if int(list[1]) > 0:
