...
class AlrajhiCardsPayrollReport(models.AbstractModel):
    _name = 'report.xlsx_payroll_report.alrajhi_cards'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        format1 = workbook.add_format({'font_size': 14, 'font_name': 'Arial', 'align': 'vcenter', 'border': 2,
                                       'bold': True, 'bg_color': '#1111ff', 'color': 'white', 'bottom': True, })
        ...
        for used_struct in used_structures:
            ...
            sheet.write(0, 1, 'Payroll Card No.', format1)
            sheet.write(0, 2, 'Employee Name', format1)
            sheet.write(0, 3, 'National ID Number', format1)
            sheet.write(0, 4, 'Amount', format1)
            sheet.write(0, 5, 'Operating Code', format1)
            sheet.write(0, 6, 'Department ID', format1)
            sheet.write(0, 11, 'Mobile Number', format1)
            for rule in rules:
                sheet.write(0, rule[0], rule[2], format1)
            sheet.write(0, 9, 'Other Earnings', format1)
            sheet.write(0, 10, 'Deductions', format1)
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        has_payslips = True
                        sheet.write(e_name, 0, slip.employee_id.employee_number or '', format3)
                        ...
                        sheet.write(e_name, 9, allowance, format3)
                        sheet.write(e_name, 10, abs(deduction), format3)

                        total_deductions += deduction
                        net += allowance
                        sheet.write(e_name, 3, slip.employee_id.identification_id or '', format3)
                        sheet.write(e_name, 4, net, format3)
                        sheet.write(e_name, 5, 'تحميل رصيد-Load Funds', format3)
                        sheet.write(e_name, 6, slip.employee_id.department_id.name or '', format3)
                        sheet.write(e_name, 11, slip.employee_id.mobile_phone  or slip.employee_id.work_phone or '', format3)
                        amount_total += net
