...
class INMAPayrollReport(models.AbstractModel):
    _name = 'report.xlsx_payroll_report.inma'
    _description = 'INMA Payroll Excel Report'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, lines):
        print(data)
        format1 = workbook.add_format({'font_size': 14, 'font_name': 'Arial', 'align': 'vcenter', 'border': 2,
                                       'bold': True, 'bg_color': '#4f81bd', 'color': 'white', 'bottom': True, })
        ...
        for used_struct in used_structures:
            ...
            sheet.write(0, 0, 'Employee #', format1)
            sheet.write(0, 1, 'Employee Name', format1)
            sheet.write(0, 2, 'Account #', format1)
            sheet.write(0, 3, 'Bank #', format1)
            sheet.write(0, 4, 'Payment Method #', format1)
            sheet.write(0, 5, 'Amount #', format1)
            sheet.write(0, 6, 'Legal #', format1)
            for rule in rules:
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        ...
                        sheet.write(e_name, 2, slip.employee_id.bank_account_id.acc_number or '', format3)
                        sheet.write(e_name, 3, slip.employee_id.bank_account_id.bank_id.name or '', format3)
                        sheet.write(e_name, 4, '', format3)
                        allowance = 0.0
                        deduction = 0.0
                        net = 0.0
                        ...
                        sheet.write(e_name, 10, deduction, format3)

                        total_deductions += deduction
                        net += allowance
                        sheet.write(e_name, 5, net, format3)
                        sheet.write(e_name, 6, slip.employee_id.identification_id or '', format3)
                        amount_total += net
